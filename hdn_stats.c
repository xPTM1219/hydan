...
void hdn_stats_embeddable_bits (hdn_data_t *data, uint32_t *num)
{
    ...
    while (curr_pos < data->sz)
    {
        ...
        ZyanStatus status = ZydisDecoderDecodeFull (&decoder,
            d.memaddr, (ZyanUSize)(data->sz - curr_pos),
            &d.insn.zydis, ops);
...
static uint32_t _count_fns (char *argv0)
{
    ...
    while (tmp_sections)
    {
        ...
        while (pos < tmp_sections->data.sz)
        {
            ...
            ZyanStatus status = ZydisDecoderDecodeFull (&decoder,
                (uint8_t *) tmp_sections->data.content + pos,
                tmp_sections->data.sz - pos,
                &insn.zydis, ops);
            ...
            if (ZYAN_SUCCESS (status))
            {
                ...
                hdn_populate_insn_from_zydis (&insn, &insn.zydis, ops);
            }
            else
            {
                insn.size = 1;
...
static void _do_stats (hdn_data_t *data, struct _insns **insns)
{
    ...
    while (curr_pos < data->sz)
    {
        ...
        if (ZYAN_SUCCESS (status))
        {
            ...
            hdn_populate_insn_from_zydis (&dis.insn, &dis.insn.zydis, ops);
        }
        else
        {
