...
frequently than others "in the wild," and this distribution must be
respected.  Global statistical distribution is not the only thing that
must be respected; ultimately local statistical distributions are as
important.  For example, procedure prologs and epilogs are usually
written with one set of instructions only, such as adding when
creating space on the stack.  Modifying any of those instructions, or
making them inconsistent across procedures, will be suspicious.

Decryption: the data embedded in the application should be hard to
retreive, even when assuming the attacker knows that there is
something embedded in the application.  This is accomplished in hydan
by using blowfish with a user-specified passphrase, and by encrypting
absolutely all embedded information -- such as message length -- to avoid
...
- Forego the sequential embedding, and instead randomize the embedding
  sequence using the passphrase as a seed
- Detect dead code, and use to embed even more data (since dead code
  can be completely re-written without affecting program execution)
- Tie instruction-to-bit mapping to the passphrase, so "add %eax,
